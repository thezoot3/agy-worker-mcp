...
/**
 * Cursors are byte offsets into `events.ndjson`.
 *
...
/**
 * Read complete lines from the tail of the file up to `maxBytes`.
 *
 * If the file is larger than `maxBytes`, reads the final `maxBytes` window
 * and discards the leading line if it was cut mid-line.
 */
export function readTailLines(filePath: string, maxBytes: number): string[] {
  ...
  try {
    const end = completeLineEnd(filePath)
    if (end === 0) return []
    ...
    if (firstLineTruncated && parts.length > 0) {
      parts.shift()
    }
