...
import { matchEnvironmentSignature } from './detect.js'
 ...
 * NDJSON stays the only stored form.
 */
...
export function normalizeEvent(event: AgyEvent, opts?: NormalizeOptions): NormalizedEvent | null {
  ...
  if (su.step_type === 'tool') {
    ...
    if (su.state === 'DONE') {
      ...
      // Same scope as `detectClass2`: only the shell runs inside the sandbox.
      const sig =
        su.tool_name === 'run_command' && typeof output === 'string' ? matchEnvironmentSignature(output) : null
      if (sig !== null) {
        return make(
          'environment_block',
          'warning',
 ...
 * This is what `agy_wait` returns as `log_tail`.
 */
