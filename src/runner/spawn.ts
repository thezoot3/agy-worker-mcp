...
export function buildAgyArgv(input: AgyArgvInput): string[] {
  if (!Array.isArray(input.addDirs) || input.addDirs.length === 0) {
    throw new ValidationError({
      ...
      value: input.addDirs,
      expected:
        'a non-empty array whose first element is the canonical workspace — required, or the workspace is never registered',
    })
  }
  ...
  if (input.inputFormat === 'stream-json' && input.prompt !== '') {
    throw new ValidationError({
      ...
      value: input.prompt,
      expected:
        'an empty string when inputFormat is stream-json — agy rejects a command-line prompt there; queue the turn on stdin instead',
    })
  }
