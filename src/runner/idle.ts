...
/**
 * Idle watchdog for `session_mode: 'session'` jobs.
 *
 ...
 * window for a job whose actual work was done in seconds.
 * `reconcile` cannot fix this on its own: it only runs from a tool entry
 * point, so nothing revisits an idle job until some client
 * happens to call one. The watchdog runs *inside the runner*, which is
 * already alive and unattended — the same reason the
 ...
 * is unmeasured territory (a *second* turn queues while a first is in flight,
 * not what an EOF mid-turn does), so this
...
export interface IdleWatchdogOptions {
  ...
  /** agy's stdin. Ending it is what makes agy exit at EOF. */
  stdin: Writable
  pollMs?: number
