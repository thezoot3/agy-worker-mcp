...
import type { JobStateFile, JudgementPacket, Lifecycle } from '../../contract/types.js'
 ...
 * (0.2.2 PR1).
 */
function describeLive(lifecycle: Lifecycle, statePath: string): string {
  const phase = readJsonIfExists<JobStateFile>(statePath)?.phase
  if (lifecycle === 'running' && phase === 'verifying') return 'job is running: agy exited, verify_command in progress'
