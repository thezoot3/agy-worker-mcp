 ...
 * denial's `required_rule` for `permissions.allow`, or the project ceiling's
 * `sandboxed`/`read_roots` for a silent sandbox block that agy reported
 * as SUCCESS. The original record behind each entry is kept verbatim in
 * `detail`.
 */
