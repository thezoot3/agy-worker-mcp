/**
 * Idle timeout (see docs/operations.md) — a `session_mode:'session'` job whose
 ...
 * `--print-timeout` window for a turn that finished in seconds.
 *
 * Also covers `agy_send` deadline behavior: `agy_send` never touches `deadline_at`.
...
describe('agy_send — deadline_at is never extended by a queued turn', () => {
