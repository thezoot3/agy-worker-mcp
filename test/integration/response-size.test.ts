/**
 * Default response stays capped while the raw log on disk is kept in full (see docs/tools.md).
 */
...
describe('capped response, uncapped raw log', () => {
  it('agy_logs honours a small max_bytes and reports truncated:true, while events.ndjson on disk holds everything', async () => {
    ...
    // Uncapped default response size stays bounded too: the
  ...
  it("agy_wait's judgement packet carries only a short log tail, never the full raw stream", async () => {
    ...
    // A single sentence, present, is what a caller needs to
