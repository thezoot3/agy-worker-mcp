/**
 * Concurrency, recovery, and process-group cleanup (see docs/operations.md) —
 * locks, stale reclaim, abnormal-exit classification, and process-group cleanup.
 */
...
describe('same write cwd concurrent start is a lock error; read-only concurrent passes', () => {
...
describe('a lock holder killed with SIGKILL is reclaimed as stale on the next call', () => {
  it('after the holder process dies, a new agy_start on the same cwd succeeds instead of conflicting', async () => {
    ...
    // Kill the underlying agy process directly (killing the lock holder,
    // i.e. the process actually holding the resource, not our own
    // process tree).
...
describe('reconcile either re-attaches a still-running job or classifies it exactly, never leaves it stuck', () => {
...
describe('cancel and timeout both leave the process group empty (pgrep -g <pgid> == 0)', () => {
