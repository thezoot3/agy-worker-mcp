/**
 * Multi-client access to one project (see docs/operations.md).
 *
 ...
 * (daemon-less design). Two clients are simulated with two independently created
...
describe('client A starts a job, client B picks it up via list/wait', () => {
  it('B sees the job in agy_list_jobs and agy_wait resolves it to completion', async () => {
    ...
    const started = replyJson(
      await handleStart(clientA, { prompt: 'check repo status', profile: 'general_worker' } as never),
    )
...
describe('the spawning client can disappear; another client still finalizes the result', () => {
  it('closing the starting client does not stop the detached runner or lose the result', async () => {
    ...
    const started = replyJson(
      await handleStart(spawner, { prompt: 'check repo status', profile: 'general_worker' } as never),
    ) as { job_id: string }
    expect(started.job_id).toBeTruthy()
    ...
    expect(result.agent_report.response.text).toContain('nothing to commit')
