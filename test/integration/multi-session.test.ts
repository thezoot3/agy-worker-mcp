/**
 * Multi-session management (see docs/operations.md).
 */
...
describe('three different sessions run concurrently, each keeping its own conversation', () => {
...
describe('a second agy_start on the same session_id while the first is still running is a lock error', () => {
...
describe('a finished oneshot job resumed via session_id keeps context', () => {
  it('the second job passes --conversation and num_turns keeps counting from the first', async () => {
    ...
    // resumed rather than starting a brand-new conversation.
...
describe('agy_send queues a follow-up turn that runs after the in-flight one, and num_turns increases', () => {
  it('a session-mode job accepts a queued turn and finishes with num_turns=2', async () => {
    ...
    // so `agy_start` seeds the prompt into `inbox.jsonl` and the
    ...
    // Queue the second turn shortly after turn 1 is in flight; it only takes
    // effect once the in-flight turn finishes (no interrupt path in print mode).
    await sleep(300)
