 ...
 * this daemon-less design — there is no in-process handle
 ...
 */
/**
 * `extraEnv` is baked straight into the wrapper script's own `export` lines —
 * this is the "existing scenario-env path" for a var that, like
 * `AGY_FAKE_SCENARIO`/`AGY_FAKE_STATE_DIR` above it, must reach the fake binary
 * without ever passing through `buildChildEnv`'s production allowlist
 * (`src/runner/spawn.ts`). `AGY_FAKE_SKIP_HOOKS` (PR3's `hook-missing`
 * scenario, `test/fake-agy/README.md`) is the first caller: it has to reach
 * `agy.mjs`'s own env read, and it must never become something production code
 * forwards.
 */
export function writeAgyWrapper(p: TestProject, scenario: string, extraEnv?: Record<string, string>): string {
  const wrapperPath = join(p.home, `agy-wrapper-${scenario}.sh`)
  const extraExports = extraEnv
...
${extraExports}exec node "${p.agyBin}" "$@"
`
 ...
 */
export function applyEnv(p: TestProject, scenario: string, extraEnv?: Record<string, string>): void {
  process.env.AGY_WORKER_HOME = p.home
  process.env.AGY_WORKER_PROJECT = p.root
  process.env.AGY_WORKER_AGY_BIN = writeAgyWrapper(p, scenario, extraEnv)
