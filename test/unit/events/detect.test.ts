...
describe('detectClass2 — silent environment block', () => {
  ...
  it('ignores tools the sandbox does not wrap: grep_search over a source tree that contains the words is not a block', () => {
    const step = toolStep({
      state: 'DONE',
      tool_name: 'grep_search',
      tool_info: { name: 'grep_search', output: 'src/x.ts:12: throw new Error("Operation not permitted")\n' },
    })
    expect(detectClass2(step)).toBeNull()
  })

  it('locates the excerpt case-insensitively (zsh: "operation not permitted")', () => {
    const step = toolStep({
      state: 'DONE',
      tool_name: 'run_command',
      tool_info: {
        name: 'run_command',
        parameters: { CommandLine: 'git status > note.txt' },
        output: 'zsh:1: operation not permitted: note.txt\n',
      },
    })
    const c2 = detectClass2(step)
    expect(c2?.signature).toBe('Operation not permitted')
    expect(c2?.excerpt).toContain('operation not permitted: note.txt')
  })

  it('matches case-insensitively and returns the canonical signature (zsh redirection error, agy 1.1.24)', () => {
    expect(matchEnvironmentSignature('zsh:1: operation not permitted: note.txt')).toBe('Operation not permitted')
    expect(matchEnvironmentSignature('mkdir: build: Operation not permitted')).toBe('Operation not permitted')
    expect(matchEnvironmentSignature('all good')).toBeNull()
  })

  it('matches each measured example (single-line or final-line)', () => {
    expect(matchEnvironmentSignature('mkdir: build: Operation not permitted')).toBe('Operation not permitted')
    expect(matchEnvironmentSignature('zsh:1: operation not permitted: note.txt')).toBe('Operation not permitted')
    expect(matchEnvironmentSignature('sh: out.txt: Operation not permitted')).toBe('Operation not permitted')
  })

  it('matches a signature on the last line of a 50-line output', () => {
    const lines = Array.from({ length: 49 }, (_, i) => `build log line ${i + 1}`)
    lines.push('mkdir: build: Operation not permitted')
    expect(matchEnvironmentSignature(lines.join('\n'))).toBe('Operation not permitted')
  })

  it('does NOT match the same signature on line 3 of a 50-line output whose tail is a vitest summary', () => {
    const lines = [
      'stdout test runner started',
      'running test suite...',
      'test stdout: mkdir: build: Operation not permitted',
      ...Array.from({ length: 44 }, (_, i) => `test case ${i + 1} passed`),
  ...
  it('ignores trailing blank lines when finding the tail of the output', () => {
    const lines = [
      'mkdir: build: Operation not permitted',
      '',
      '   ',
      '\t',
      '',
