...
import { readLinesFrom, readFirstLine, tailLines, readTailLines } from '../../../src/events/cursor.js'
...
describe('readTailLines', () => {
  it('returns all complete lines when the file is smaller than maxBytes', () => {
    writeFileSync(file, '{"i":0}\n{"i":1}\n{"i":2}\n')
    const lines = readTailLines(file, 1000)
    expect(lines).toEqual(['{"i":0}', '{"i":1}', '{"i":2}'])
  })

  it('returns only the last lines excluding the truncated first line when the file is larger than maxBytes', () => {
    // 8 bytes per line: '{"i":0}\n'
    writeFileSync(file, '{"i":0}\n{"i":1}\n{"i":2}\n{"i":3}\n{"i":4}\n')
    // 5 lines * 8 bytes = 40 bytes.
    // If maxBytes is 18 bytes:
    // start = 40 - 18 = 22.
    // Line 0 is 0..7, Line 1 is 8..15, Line 2 is 16..23.
    // Offset 22 is in the middle of Line 2.
    // Line 2 is truncated and dropped.
    // Lines 3 ('{"i":3}') and 4 ('{"i":4}') are returned.
    const lines = readTailLines(file, 18)
    expect(lines).toEqual(['{"i":3}', '{"i":4}'])
  })

  it('returns empty array for missing or empty file', () => {
    expect(readTailLines(join(dir, 'missing.ndjson'), 100)).toEqual([])
    writeFileSync(file, '')
    expect(readTailLines(file, 100)).toEqual([])
  })
})
