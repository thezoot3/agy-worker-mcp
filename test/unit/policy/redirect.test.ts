...
describe('the gate denies an escaping redirect under EVERY profile', () => {
  // Measured live 2026-09-01: `printf hello > /tmp/x` ran to
  ...
  it.each<[Profile, string, string]>([
    ...
    ['general_worker', 'echo evil > ~/.zshrc', 'deny'],
    ['general_worker', 'echo x > $HOME/foo.txt', 'deny'],
  ...
  it('echo evil > ~/.zshrc is denied by write containment', () => {
    const outcome = decide({
      payload: {
        conversationId: 'conv-1',
        toolCall: { name: 'run_command', args: { CommandLine: 'echo evil > ~/.zshrc' } },
      },
      bound: boundFor('general_worker'),
    })
    expect(outcome.decision.decision).toBe('deny')
    expect(outcome.log?.policy).toBe('containment')
    expect(outcome.decision.reason).toContain('.zshrc')
  })

  it('echo x > $HOME/out.txt is denied with unexpanded_path guidance', () => {
    const outcome = decide({
      payload: {
        conversationId: 'conv-1',
        toolCall: { name: 'run_command', args: { CommandLine: 'echo x > $HOME/out.txt' } },
      },
      bound: boundFor('general_worker'),
    })
    expect(outcome.decision.decision).toBe('deny')
    expect(outcome.log?.policy).toBe('containment')
    expect(outcome.decision.reason).toContain('unexpanded_path')
  })

  it('echo hi > out.txt stays in workspace and is not denied by containment', () => {
    const outcome = decide({
      payload: {
        conversationId: 'conv-1',
        toolCall: { name: 'run_command', args: { CommandLine: 'echo hi > out.txt' } },
      },
      bound: boundFor('general_worker'),
    })
