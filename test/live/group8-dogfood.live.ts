/**
 * Group 8 — Dogfooding: two realistic tasks.
 *
 * Previous live tests all used synthetic prompts ("Reply with exactly: PONG",
 * inducing "run python3", etc.). Here we run two realistic, useful tasks and check
 * whether `agy_result`'s response is genuinely readable by a human or agent:
 *
 *  - L15: workspace structure summary using `research_readonly` (investigation)
 *  - L16: creating a single file using `general_worker`, verifying `verified_success`
 *    is produced with `expected_artifacts`
 */
