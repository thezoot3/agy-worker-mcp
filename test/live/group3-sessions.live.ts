/**
 * Group 3 — Sessions. Verifies whether `--conversation` resume is lossless and
 * whether turns queued via `agy_send` actually execute. These two properties are
 * prerequisites for the "turn = process" design (see docs/operations.md).
 */
...
live('L9 — agy_send queues a turn onto a live session and num_turns advances', () => {
  it('a session-mode job accepts a follow-up and finishes having run both turns', async () => {
    ...
    // write to (the inbox is drained by the runner, not the server).
    await waitUntil(
      async () => {
