/**
 * Group 1 — Happy path. If this is broken, the remaining groups are meaningless.
 *
 * Targets: A1 (`--print-timeout` syntax), A2 (`ps -o lstart=`), A6
 * (`--print=<prompt>` + `--input-format stream-json` coexistence).
 */
...
live('L1 — oneshot smoke: the argv we build is accepted and the run completes', () => {
  it('agy accepts --print-timeout/--add-dir/--output-format and reaches success_unverified', async () => {
    ...
    // — the gate's whole binding strategy rests on this.
